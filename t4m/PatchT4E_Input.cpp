...
// keyNum_t defined in cod/enums.hpp under T4::, exposed via `using namespace T4;` from T4.h.
// (Local copy removed during header consolidation; cod/enums.hpp lacks K_NUMPAD0=0x60
// but it is unreferenced in this codebase so removing the local enum is safe.)
...
void AimAssist_ClearAutoAimTarget(const T4::AimInput* input) {

	auto& aaGlob = T4::aaGlobArray[input->localClientNum];
...
void AimAssist_SetAutoAimTarget(T4::AimAssistGlobals* aaGlob, const T4::AimScreenTarget* screenTarget)
{
	AimAssist_ClearAutoAimTarget(aaGlob);
...
const T4::AimScreenTarget* AimAssist_GetBestTarget(const T4::AimAssistGlobals* aaGlob, const float range, const float regionWidth, const float regionHeight)
{
...
void AimAssist_ApplyAutoAim(const T4::AimInput* input, T4::AimOutput* output)
{
	assert(input);
	assert(output);

	auto& aaGlob = T4::aaGlobArray[input->localClientNum];
	// `::bg_weaponDefs` (defined in PatchT4Script.cpp) is updated at runtime by
	// SetupBgWeaponDefsTable to point at the relocated heap table.
	const auto* weaponDef = ::bg_weaponDefs[T4::CG_GetPlayerWeapon(input->ps, input->localClientNum)];
	...
	if ((input->ps->weaponstate >= T4::WEAPON_RELOADING && input->ps->weaponstate <= T4::WEAPON_RELOAD_END))
	{
		return;
	}
	...
	if (AimAssist_IsAutoAimActive(input->localClientNum, input))
	{
		...
		{
			if (input->ps->weaponstate != T4::WEAPON_RELOADING
				&& input->ps->weaponstate != T4::WEAPON_RELOADING_INTERUPT
				&& input->ps->weaponstate != T4::WEAPON_RELOAD_START
				&& input->ps->weaponstate != T4::WEAPON_RELOAD_START_INTERUPT
				&& input->ps->weaponstate != T4::WEAPON_RELOAD_END)
			{
				if (weaponDef->aimDownSight)
				{
...
bool AimAssist_IsPlayerUsingOffhand(const Game::AimInput* input)
{
	...
	// `::bg_weaponDefs` (PatchT4Script.cpp) is updated to the relocated heap table.
	const auto* weaponDef = ::bg_weaponDefs[input->ps->weapon];
...
void __cdecl AimAssist_ApplyAutoMelee_4_ApplyAutoAim(T4::AimInput* a1, T4::AimOutput* a2) {
...
void PatchT4E_Input() {
	gpad_lastinput = T4::Dvar_RegisterInt(1, "gpad_lastinput", 1, 2, DVAR_FLAG_ROM, "Returns what was last input by player\n1 = Mouse\n2 = Gamepad");

	gpad_autoaim_enabled = T4::Dvar_RegisterBool(true, "gpad_autoaim_enabled", DVAR_FLAG_SAVED | DVAR_FLAG_ARCHIVE);

	gpad_lockon_enabled = T4::Dvar_RegisterBool(true, "gpad_lockon_enabled", DVAR_FLAG_SAVED | DVAR_FLAG_ARCHIVE);


	aim_autoAimRangeScale = T4::Dvar_RegisterFloat("aim_autoAimRangeScale", 1.f, 0.f, 2.f,0);

	aim_lockon_pitch_strength = T4::Dvar_RegisterFloat("aim_lockon_pitch_strength", 0.6f, 0.f, 1.f, 0);
